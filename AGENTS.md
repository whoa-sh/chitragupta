...
This is a Kotlin Spring Boot service named `chitragupta`.

- `src/main/kotlin/sh/whoa/chitragupta/` contains application source code. The current entry point is `ChitraguptaApplication.kt`.
- `src/main/resources/` contains runtime configuration, including `application.properties`.
- `src/test/kotlin/sh/whoa/chitragupta/` contains JUnit 5/Spring Boot tests.
...
Keep package paths aligned with `sh.whoa.chitragupta`. Add new code under feature-oriented packages as the service grows, such as `controller`, `service`, `repository`, or domain-specific names.
