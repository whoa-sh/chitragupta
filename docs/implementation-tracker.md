...
| T4 | Harden Docker and Compose runtime | done | Non-root + locked-down container options without startup regression | `docker compose config` valid, `docker compose up --build -d` and `docker compose ps` showed healthy app + DB | pending | Added read-only FS, tmpfs, dropped caps, and no-new-privileges |
